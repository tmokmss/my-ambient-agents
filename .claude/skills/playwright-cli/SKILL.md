...
npx After each command, playwright-cli provides a snapshot of the current browser state.
...
npx You can also take a snapshot on demand using `playwright-cli snapshot` command.
...
npx In some cases user might want to install playwright-cli locally. If running globally available `playwright-cli` binary fails, use `npx playwright-cli` to run the commands. For example:

```bash
npx npx playwright-cli open https://example.com
npx npx playwright-cli click e1
```
