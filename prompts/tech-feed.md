...
- 優先する: 新サービス・新機能、新ランタイム／新言語サポート、既存サービスの挙動が変わる機能追加、GA / Preview への昇格、性能・上限・課金体系の変更など、開発者の実装や設計判断に影響するもの
- 低優先とする: 既存サービス・既存インスタンスタイプが新しいリージョンで使えるようになるだけの告知（`now available in ... Region(s)` / `additional regions` / `more regions` 系）。GovCloud (US) や European Sovereign Cloud への展開も、機能追加を伴わない限り同じ「提供範囲の拡大のみ」として扱う

判断はタイトルのキーワードによる機械的な除外ではなく、タイトルと description から読み取れる主題で行うこと
（リージョン展開と同時に新機能や新インスタンスタイプの GA を告知しているエントリは、機能アップデートとして対象に含めてよい）。
機能アップデート系の候補が十分にある限り、リージョン展開系で枠を埋めないこと。リージョン展開系を完全に禁止するわけではなく、機能アップデート系の候補が枠に満たない場合に限り掲載してよい。
それでも候補が3件に満たない場合は、無理に枠を埋めず件数を減らしてよい。
...
tech-feed は1日2回実行されるため、ファイル件数で直近3件を取ると当日・前日分だけで埋まり、それ以前が参照されなくなる。これを避けるため件数ではなく日数で範囲を取っている。
...
1. **URL の機械的な突合（一次フィルタ）**: 各ソースから集めた候補の URL を、`/tmp/past-index.tsv` の URL 列と完全一致で突き合わせ、一致したものは候補から機械的に除外する。
   はてなブックマークの候補は元記事 URL で突き合わせること（インデックスの URL 列には元記事 URL が入り、はてブページ URL は含まれない）。
2. **タイトルベースの類似トピック判定（二次フィルタ）**: 一次フィルタを通過した候補について、`/tmp/past-index.tsv` のタイトル列と照らし合わせ、
   過去に取り上げたネタと同一または非常に類似したトピックを除外する。
   同じ話題の別記事、同一事件の続報、一次情報とその解説記事などは、URL が異なっていても重複とみなす。
