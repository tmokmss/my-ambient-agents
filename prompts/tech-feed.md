...
1. **URL の機械的な突合（一次フィルタ）**: 各ソースから集めた候補の URL を、上記出力の URL 列と完全一致で突き合わせ、一致したものは候補から機械的に除外する。
   はてなブックマークの候補は元記事 URL で突き合わせること（出力の URL 列には元記事 URL が入り、はてブページ URL は含まれない）。
2. **タイトルベースの類似トピック判定（二次フィルタ）**: 一次フィルタを通過した候補について、上記出力のタイトル列と照らし合わせ、
