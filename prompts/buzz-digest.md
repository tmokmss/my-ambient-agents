...
src/content/reports/ ディレクトリ以下を再帰的に検索し、ファイル名に "${AGENT_SLUG}" を含むものをパスの降順でソートし、過去3日分（実行回数によらず本日を含む直近3日付の日付ディレクトリに含まれる該当ファイルすべて、最大10件まで）を読み込む。
1日複数回実行時に直近3件がすべて当日分となり前日以前のレポートが参照されなくなる問題を避けるため、件数ではなく日数で範囲を取る。
