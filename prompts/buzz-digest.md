...
同じURL、同じ話題の別記事、同一事件の続報、同一YouTube video ID、同一作品の別言語版・別バージョンのトレーラーなどは重複とみなす。
（別言語版・別バージョンの判定は「同一レポート内の重複（YouTube US / JP）」の (a)/(b) と同じ基準を用いる。
video ID が違っていても、掲載済みの動画と同じ作品・同じイベントを扱う言語違い／字幕違い／尺違いの動画は新規候補にしない。）
...
**単に同じフランチャイズ名を含むだけでは同一グループにしてはならない。**
